...
public interface MeActivityRepository extends ActivityRepository {
    ...
    @Query(value = """
            SELECT a.id, a.member_created_id, a.member_id, a.project_id, a.todo_id,
             a.todo_list_id, a.content_action, a.url_image, a.created_date FROM activity_todo a 
    ...
    @Query(value = """
            SELECT * FROM activity_todo a 
            WHERE a.image_id = :idImage
            """, nativeQuery = true)
    ActivityTodo findActivityByIdImage(@Param("idImage") String idImage);

    @Query(value = """
            SELECT * FROM activity_todo a 
            WHERE todo_id = :idTodo AND content LIKE :action
            """, nativeQuery = true)
    ActivityTodo findActivityByAction(@Param("idTodo") String idTodo, @Param("action") String action);
}
