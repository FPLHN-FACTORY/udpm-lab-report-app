...
public interface StAttendanceRepository extends AttendanceRepository {
    @Query(value = """
            SELECT  m.name as name, m.meeting_date as meetingDate, m.meeting_period as meetingPeriod, a.status as status
             from student_classes sc
             join class c on sc.class_id = c.id
             join meeting m on m.class_id = c.id
             join attendance a on a.meeting_id = m.id
             where a.student_id = :#{#id}
             group by m.name , m.meeting_date , m.meeting_period , a.status
             """, nativeQuery = true)
