 ...
 */
@Repository
public interface TeTeamFactoryRepository extends TeamFactoryRepository {
    ...
    @Query(value = """
            ...
            WHERE ( :#{#req.name} IS NULL 
                    ...
                    ORDER BY obj.created_date DESC       
            """, nativeQuery = true)
    Page<TeTeamFactoryResponse> searchTeam(@Param("req") TeFindTeamFactoryRequest req, Pageable page);
    ...
    @Query(value = """
            ...
            WHERE c.id = :idTeam
            """, nativeQuery = true)
    List<TeTemplateMemberFactoryResponse> getMemberTeamFactory(@Param("idTeam") String idTeam);
