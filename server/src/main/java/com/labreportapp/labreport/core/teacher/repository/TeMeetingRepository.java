...
public interface TeMeetingRepository extends JpaRepository<Meeting, String> {
    ...
    @Query(value = """
            SELECT ROW_NUMBER() OVER(ORDER BY m.meeting_date DESC) AS stt,
                 ...
                 m.type_meeting AS type_meeting,
                 COALESCE(m.address, '') AS address_meeting,
             ...
             WHERE m.teacher_id = :#{#req.idTeacher} 
                AND
                    DATE(CONVERT_TZ(FROM_UNIXTIME(m.meeting_date / 1000), 'UTC', 'Asia/Ho_Chi_Minh')) = :#{#req.dateNow}
    ...
    // DATE(CONVERT_TZ(CURRENT_DATE(), 'UTC', 'Asia/Ho_Chi_Minh'))
    @Query(value = """
