...
//   import "./style-detail-team-factory.css";
  ...
  import {
    ...
    SearchTeDetailTeam,
    SetStDetailTeam,
  } from "../../../../app/student/StDetailTeamFactorySlice.reducer";
  ...
  const TeDetailTeamFactory = () => {
    ...
    useEffect(() => {
      ...
      return () => {
        dispatch(SetStDetailTeam([]));
      };
    }, []);
  
    const loadData = () => {
        StTeamFactoryAPI.detailTeam(id).then((response) => {
        setDetailTeam(response.data.data);
      });
    };
  
    const loadDataMemberTeamFactory = () => {
        StTeamFactoryAPI.detailMemberTeamFactory(id).then((response) => {
        dispatch(SetStDetailTeam(response.data.data));
        setLoading(false);
      });
    };
  
    const data = useAppSelector(GetStDetailTeam);
    ...
    const loadDataAllMemberFactory = () => {
        StTeamFactoryAPI.getAllMemberFactory().then((response) => {
      ...
      <div className="box-general" style={{ paddingTop: 50 }}>
        ...
        <div className="title_activity_management" style={{ marginTop: 0 }}>
          ...
          <span style={{ marginLeft: "10px" }}>
            <Link to="/student/team-factory" style={{ color: "black" }}>
              {" "} 
              Danh sách team
            </Link>{" "}
