...
import { StMemberFactoryAPI } from "../../../api/student/StMemberFactoryAPI";
import { useAppDispatch, useAppSelector } from "../../../app/hook";
import {
  GetStMemberFactory,
  SetStMemberFactory,
} from "../../../app/student/StMemberFactorySlice.reducer";
...
const TeMemberFactory = () => {
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Thành viên trong xưởng | Lab-Report-App";

    loadDataRoles();
    return () => {
      dispatch(SetStMemberFactory([]));
    };
  }, []);
  ...
  const loadData = () => {
    ...
    StMemberFactoryAPI.getPage(filter).then((response) => {
      dispatch(SetStMemberFactory(response.data.data.data));
  ...
  const loadDataRoles = () => {
    StMemberFactoryAPI.getRoles().then((response) => {
      setRoles(response.data.data);
    });
    StMemberFactoryAPI.getTeams().then((response) => {
      setTeams(response.data.data);
    });
    StMemberFactoryAPI.getNumberMemberFactory().then((response) => {
