...
const ModalUpdatePeriod = ({ visible, onCancel, idPeriod }) => {
  ...
  const fetchData = () => {
    PeriodProjectAPI.detail(idPeriod).then((response) => {
      ...
      setName(obj.name);
      setStartTime(
        moment(obj.startTime).utcOffset(7).format("YYYY-MM-DDTHH:mm")
      );
      setEndTime(moment(obj.endTime).utcOffset(7).format("YYYY-MM-DDTHH:mm"));
...
export default ModalUpdatePeriod;
